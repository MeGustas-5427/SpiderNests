...
class MongoClient:
    ...
    def FindAndModify(self,collection,query,change):
        try:
            self.dbc[collection].find_and_modify(query, {"query": query, "update": change})
            self.logger.info('FindAndModify -> ' + str(query) + ' from ' + collection + ' Succeed')
        except:
            self.logger.warn('FindAndModify -> ' + str(query) + ' from ' + collection + ' Failed')
